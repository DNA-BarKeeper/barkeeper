...
class SpecimenExporter < ApplicationRecord
  ...
  def xml_file(file, project_id)
    markers = Marker.in_project(Project.find_by_name('GBOL5'))
    ...
    xml.comment!("Generated by a Barcode Workflow Manager web app on #{Time.zone.now}")
        ...
        xml.Table do
          ...
          Individual.includes(species: :family).in_project(project_id).find_each do |individual|
            xml.Row do
              ...
              cell_tag(xml, '') # Sex
              cell_tag(xml, "#{ENV['PROJECT_DOMAIN']}/individuals/#{individual.id}/edit") # evtl. Bemerkungen zur Probe
              ...
              markers.each do |marker|
                ...
                url = longest_sequence&.contigs&.first&.id ? "#{ENV['PROJECT_DOMAIN']}/contigs/#{longest_sequence&.contigs&.first&.id}/edit" : ''
                cell_tag(xml, url)
