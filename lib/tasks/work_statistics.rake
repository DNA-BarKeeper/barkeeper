...
namespace :data do
  ...
  task :work_statistics => :environment do
    ...
    newly_verified_contigs = Contig.where(:verified_at => range)
    ...
    new_primer_reads = PrimerRead.where(:created_at => range)
    ...
    new_isolates = Isolate.where(:created_at => range)
    ...
    all_isolates_count = Isolate.all.count
    isolates_bonn_cnt = Isolate.where("lab_nr ilike ?", "%gbol%").length
    isolates_berlin_cnt = Isolate.where("lab_nr ilike ?", "%db%").length
